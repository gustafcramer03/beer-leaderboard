...
import type { default as ConfettiFn } from "canvas-confetti";
...
//
// canvas-confetti (~30KB) is loaded lazily on first use so it stays out of the
// initial bundle for sessions that never trigger a celebration.

const GOLD = ["#f59e0b", "#fbbf24", "#fde68a", "#ffffff"];

let confettiPromise: Promise<typeof ConfettiFn> | null = null;
function getConfetti(): Promise<typeof ConfettiFn> {
